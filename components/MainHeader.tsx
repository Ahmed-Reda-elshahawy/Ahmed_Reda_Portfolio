            ...
            <ul className="bg-zinc-200/95 border border-zinc-400/50 px-2 shadow-md flex items-center justify-center gap-4 flex-wrap py-3 box-border sm:gap-6 sm:w-fit sm:mx-auto sm:my-4 sm:px-8 sm:rounded-full">
                {
                    Links.map((link, i) => (
                        <li key={i}>
                            ...
                            }} className={`font-medium text-lg transition-all ${activeSection === link.linkName ? 'text-slate-700 py-2 px-4 bg-zinc-300/80 rounded-full' : 'text-slate-600'}`} href={link.linkPath}>
                                {link.linkName}
                            </Link>
                        </li>
                    ))
